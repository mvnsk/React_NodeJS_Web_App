...
module.exports = function request(keywords, timePeriod, cbFunc) {
	...
	return Promise.all(promiseArr(obj.keywords, obj.timePeriod)).then(function (results) {
...
function promiseArr(keywords, timePeriod) {
	return groupKeywords(keywords).map(function (keyword, index, arr) {
		return rp('http://www.google.com/trends/fetchComponent?q=' + keyword + '&cid=TIMESERIES_GRAPH_0&export=3&' + timePeriod).then(function (htmlString) {
